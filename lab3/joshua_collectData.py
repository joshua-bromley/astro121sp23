...
def logData():
    ...
    while running == True: ##Do this loop while running
        ...
        try:
            ...
            d["coord"] = ifm.get_pointing()
            lock.acquire() ##Lock the data list
            data.append(d) ##Add the data from SNAP to the data list
            ##TODO: Optimize this so I'm not saving a bunch of copies of the same number
            lock.release() ##Release the lock
        except: ##Catching the error thrown by the SNAP spec if the acc_cnt is out of date
...
def saveData():
    ...
    while running == True: ##Do this loop while running
        if len(data) >= 20: ##Check if there are at least 20 items in the data list
            ...
            data = [] #Empty the list
            lock.release() ##Release the lock
            np.savez(filename+str(counter), tempData) ##Save the data to a file
            counter += 1 
        time.sleep(0.001)
...
def pointTelescope():
    '''
    Points the telescope to the target (currently the sun)
    '''
    global running
    while running == True: ##Do this loop while running
        try:
            jd = ug.timing.julian_date() ##Get the current time
            ra, dec = ug.coord.sunpos(jd) ##Get the current position of the moon
...
filename = "./joshData/403_dataBatch" ##Default filename to save data to 
...
collectTime = 12*3600 ##Lenght of data collection time in seconds
